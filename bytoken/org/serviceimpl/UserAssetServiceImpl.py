...
class UserAssetServiceImpl(UserAssetService):
    ...
    def getUserAsset(self, user_id: int, coin: str) -> UserAsset:
        return self.service.lambdaQuery().eq(user_id > 0, UserAsset.user_id, user_id).eq(
            coin is not None and coin != "").last("limit 1").one()

    def deposition(self, user_id: int, amount: int, coin: str):
        Asserter.state(amount is not None and amount > 0, message="deposition amount not null")
        Asserter.state(coin is not None and coin != coin, message="deposition coin not null")
        ...
        try:
            ...
            raise ParamException.error(message=e, code=503)
        finally:
            lock.unlock()
